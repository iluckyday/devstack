...
sudo chroot $TARGET_ROOT systemctl -f mask apt-daily.timer apt-daily-upgrade.timer fstrim.timer motd-news.timer cron.service logrotate.timer
...
disk-image-create -o /tmp/devstack vm block-device-mbr cleanup-kernel-initrd devuser devstack debian-minimal
