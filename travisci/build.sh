...
LIBGUESTFS_BACKEND_SETTINGS=force_tcg qemu-system-x86_64 -name devstack-building -machine q35,accel=kvm -cpu host,pmu=off -smp "$(nproc)" -m 6G -nographic -object rng-random,filename=/dev/urandom,id=rng0 -device virtio-rng-pci,rng=rng0 -boot c -drive file=/tmp/devstack.raw,if=virtio,format=raw,media=disk -netdev user,id=n0,ipv6=off -device virtio-net,netdev=n0
