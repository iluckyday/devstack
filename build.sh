...
DEBIAN_FRONTEND=noninteractive apt-get update 2>&1 >/dev/null && DEBIAN_FRONTEND=noninteractive apt-get install -y sudo procps python3-diskimage-builder qemu-system-x86 2>&1 >/dev/null
...
/usr/bin/qemu-system-x86_64 -machine q35 -m 3G -device intel-iommu -display none -object rng-random,filename=/dev/urandom,id=rng0 -device virtio-rng-pci,rng=rng0 -boot c -drive file=/tmp/devstack.qcow2,if=virtio,format=qcow2,media=disk
