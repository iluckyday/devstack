...
debootstrap --no-check-gpg --no-check-certificate --components=main,universe,restricted,multiverse --variant minbase --include "$base_apps" ${LTS_LATEST_NAME} ${mount_dir}
...
deb https://mirrors.ustc.edu.cn/ubuntu/ ${LTS_LATEST_NAME}-security main restricted universe multiverse
deb https://mirrors.ustc.edu.cn/ubuntu/ ${LTS_LATEST_NAME}-updates main restricted universe multiverse
deb https://mirrors.ustc.edu.cn/ubuntu/ ${LTS_LATEST_NAME}-backports main restricted universe multiverse
deb https://mirrors.ustc.edu.cn/ubuntu/ ${LTS_LATEST_NAME}-proposed main restricted universe multiverse
EOF
...
echo Converting ...
qemu-img convert -f raw -c -O qcow2 /tmp/devstack.raw /dev/shm/devstack-tacker-aio-init-cn.img

echo "Compressed image size:"
du -h /dev/shm/devstack-tacker-aio-init-cn.img

echo 'qemu-system-x86_64 -name devstack-building -machine q35,accel=kvm:hax:hvf:whpx:tcg -cpu kvm64 -smp "$(nproc)" -m 4G -nographic -object rng-random,filename=/dev/urandom,id=rng0 -device virtio-rng-pci,rng=rng0 -boot c -drive file=/tmp/devstack.img,if=virtio,format=qcow2,media=disk -netdev user,id=n0,ipv6=off -device virtio-net,netdev=n0'
